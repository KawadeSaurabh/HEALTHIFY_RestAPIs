...
public class AdminController {
	...
	private UserService userService;
	 ...
	 @PostMapping(value = "/add-user", produces = "application/json")
	    public ResponseEntity<Object> registerUser(@RequestBody @Valid User newUser) {
	        // Convert SpecialtyRequestDtos to Specialty entities
	        Set<Specialty> specialties = new HashSet<>();
	        for (Specialty specialtyRequestDto : newUser.getSpecialties()) {
	            Specialty specialty = new Specialty();
	            specialty.setName(specialtyRequestDto.getName());
	            specialty.setUser(specialtyRequestDto.getUser());
	            specialties.add(specialty);
	        }

	        // Set the user for each specialty
	        specialties.forEach(specialty -> specialty.setUser(newUser));
	        ...
	        if (isAdded) {
	            ...
	            return new ResponseEntity<>("Successful Addition of the user", HttpStatus.CREATED);
	        } else {
	            LOG.info("User Already Exists With ID: " + newUser.getUsername());
	            throw new ResourceAlreadyExistsException("User Already Exists With ID: " + newUser.getUsername());
	        }
	    }
...
//				return new ResponseEntity<Object>("Successful Addition of the user", HttpStatus.CREATED);
//			} else {
